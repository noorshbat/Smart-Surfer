...
class RecorderFileJson {
    ...
    internal enum Constants {
        ...
        static let firstData: Data = String("{\"data:\": [").data(using: .utf8)!
        static let lastData: Data = String("]}").data(using: .utf8)!
    }
    ...
    private var latestMovesenseEvent: Data?
    ...
    func startRecording(_ date: Date) {
        streamQueue.sync {
            ...
            streamFileHandle?.write(Constants.firstData + Constants.lfData)
    ...
    func stopRecording(_ date: Date) {
        streamQueue.sync {
            ...
            if let prevData = self.latestMovesenseEvent {
                streamFileHandle?.write(prevData + Constants.lfData + Constants.lastData)
            }
...
extension RecorderFileJson: Observer {
    ...
    func encodeEvent(_ event: MovesenseEvent) {
        ...
        if let prevData = self.latestMovesenseEvent {
            streamQueue.sync {
                streamFileHandle?.write(prevData + Constants.commaData + Constants.lfData)
            }
        }
        self.latestMovesenseEvent = data
    }
}
