...
public class AccMultiCsvLogger {

    private final String TAG = AccMultiCsvLogger.class.getSimpleName();
    public static final int REQUEST_WRITE_EXTERNAL_STORAGE_PERMISSION = 99;
    public File file;
    public FileWriter fileWriter;
    private boolean isHeaderExists = false;

    public AccMultiCsvLogger() {
        try {
            file = createLogFile();
    ...
    public File createLogFile() {
        if (isExternalStorageWritable()) {
            File externalDirectory = Environment.getExternalStorageDirectory();
            File appDirectory = new File(externalDirectory, "Movesense");
            File logFile = new File(appDirectory, "AccMultiSensorUsageActivity");
